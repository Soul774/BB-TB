...
scanRange = 50 # raise if miss in shiftlock
targetColor = (0, 255, 0) # green #00FF00v
...
while True:
    ...
    if toggle:
        ...
        green_detected = any(
            isColorClose(pixelColor(x + dx, y + dy), targetColor, tolerance)
            for dx, dy in [(-scanRange, 0), (scanRange, 0), (0, -scanRange), (0, scanRange)]
        )
        if green_detected:
            mouse.click('left')
            time.sleep(0.1)
